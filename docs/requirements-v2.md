...
### 6.4.1 テンプレートとインスタンスの用語

繰り返し利用される元データ（テンプレート）と、それを使った実施記録（インスタンス）の関係を以下の三層で呼び分ける。

| 層 | 親（元） | 子（生成物） | 関係を示すフィールド |
| --- | --- | --- | --- |
| 抽象概念 | テンプレート (template) | インスタンス (instance) | `instance_of` |
| type 名（meta.json） | `checklist`, `routine` | `log` | `instance_of` |
| UI 表示（ユーザー向け） | 「チェックリスト」 | 「実施記録」 | リンクで表示 |
| コード（クラス／識別子） | `Checklist` 等 | `ChecklistInstance` / `WorkLog` | `InstanceOf` プロパティ |

操作名:

| 場所 | 用語 |
| --- | --- |
| ボタンラベル | 「実施を開始」（または「新規記録」） |
| メニュー項目 | 「テンプレートから実施記録を作成」 |
| メソッド名 | `CreateInstance(templateId)` |

「使う」は曖昧（閲覧も含意しうる）なので避ける。「実施を開始」が動作として明確。

`type=log` は「自由形式の作業ログ」と「チェックリスト実施記録」の両方を兼ねる。両者は `instance_of` の有無で区別する。将来的に他テンプレート種別が増えたら細分化を検討する（§16）。
